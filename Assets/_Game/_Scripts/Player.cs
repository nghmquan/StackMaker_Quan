...
public class Player : MonoBehaviour
{
    ...
    private void CheckLastBrick(Direction _direction)
    {
        ...
        {
            var finalPos = GetTargetPosition(_direction);
            var direction = finalPos - transform.position;
            var checkPosition = transform.position - direction;
            Physics.Raycast(checkPosition + new Vector3(0, .5f, 0), Vector3.down, out hit, brickLayer);

            if (hit.collider != null && moveCount > 0)
            {
                var brick = hit.collider.gameObject;

                if (collectedList.Contains(brick) == false)
                {
                    if (brick.transform.GetChild(0).gameObject.activeSelf == true)
                    {
                        playerBrick.AddBrick();
                        TurnOffDimian(brick);
                    }
                    else if (brick.transform.GetChild(0).gameObject.activeSelf == false)
                    {
                        playerBrick.RemoveBrick();
                        TurnOnDinin(brick);
                    }
                    collectedList.Add(brick);
                }
            }
